#!/bin/bash
# Power-chops Teak trees on the Isle of Souls (members-only, safe) until a
# target level is reached (default 70). Drops teak logs on a full inventory.
# Reaches the island via the Soul Wars minigame teleport through the walker.
# Auto-recovers from disconnects via relogin_recovery.sh pinned to a members
# world since this spot is members-only territory.
#
# Usage: woodcutting_train_teaks.sh [target_level]
...
for cycle in $(seq 1 50000); do
  if [ $((cycle % 10)) -eq 0 ]; then
    "$DIR/relogin_recovery.sh" 535 "$TEAK_X" "$TEAK_Y" 0
  fi
  ...
  if [ "$full" = "True" ]; then
    curl -s -X POST -H "X-Agent-Token: $TOKEN" -H "Content-Type: application/json" \
      -d '{"name":"Teak logs","all":true}' "$BASE/inventory/drop" > /dev/null
  ...
  curl -s -X POST -H "X-Agent-Token: $TOKEN" -H "Content-Type: application/json" \
    -d '{"name":"Teak tree","action":"Chop down"}' "$BASE/objects/interact" > /dev/null
