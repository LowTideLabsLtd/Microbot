#!/bin/bash
# Trains Woodcutting on Oak trees around Lumbridge until a target level is
# reached (default 35). Drops oak logs when the inventory fills since banking
# is not part of this powermining-style loop. Auto-recovers from disconnects
# via relogin_recovery.sh.
#
# Usage: woodcutting_train_oaks.sh [target_level]
...
for cycle in $(seq 1 6000); do
  ...
  if [ "$full" = "True" ]; then
    curl -s -X POST -H "X-Agent-Token: $TOKEN" -H "Content-Type: application/json" \
      -d '{"name":"Oak logs","all":true}' "$BASE/inventory/drop" > /dev/null
    for extra in "Logs" "Bird nest" "Bird nest (empty)" "Oak leaves" "Leaves"; do
      curl -s -X POST -H "X-Agent-Token: $TOKEN" -H "Content-Type: application/json" \
  ...
  curl -s -X POST -H "X-Agent-Token: $TOKEN" -H "Content-Type: application/json" \
    -d '{"name":"Oak tree","action":"Chop down"}' "$BASE/objects/interact" > /dev/null
